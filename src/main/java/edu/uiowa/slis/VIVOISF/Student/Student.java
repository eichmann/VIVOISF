...
public class Student extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static Student currentInstance = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			StudentIterator theStudentIterator = (StudentIterator) findAncestorWithClass(this, StudentIterator.class);

			if (theStudentIterator != null) {
				subjectURI = theStudentIterator.getSubjectURI();
				label = theStudentIterator.getLabel();
			}

			if (theStudentIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in Student doStartTag", e);
			throw new JspTagException("Exception raised in Student doStartTag");
		} finally {
			freeConnection();
		}
