package edu.uiowa.slis.VIVOISF.ResearchOrganization;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class ResearchOrganizationHasEquipment extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static ResearchOrganizationHasEquipment currentInstance = null;
	private static final Log log = LogFactory.getLog(ResearchOrganizationHasEquipment.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			ResearchOrganizationHasEquipmentIterator theResearchOrganizationHasEquipmentIterator = (ResearchOrganizationHasEquipmentIterator)findAncestorWithClass(this, ResearchOrganizationHasEquipmentIterator.class);
			pageContext.getOut().print(theResearchOrganizationHasEquipmentIterator.getHasEquipment());
		} catch (Exception e) {
			log.error("Can't find enclosing ResearchOrganization for hasEquipment tag ", e);
			throw new JspTagException("Error: Can't find enclosing ResearchOrganization for hasEquipment tag ");
		}
		return SKIP_BODY;
	}
}

