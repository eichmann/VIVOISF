...
public class SecurityIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static SecurityIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(SecurityIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://www.w3.org/2006/vcard/ns#Security> . "+
			...
			log.error("Exception raised in SecurityIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in SecurityIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in SecurityIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in SecurityIterator doAfterBody");
		}
