...
public class EducationalProcessIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static EducationalProcessIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(EducationalProcessIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://vivoweb.org/ontology/core#EducationalProcess> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
