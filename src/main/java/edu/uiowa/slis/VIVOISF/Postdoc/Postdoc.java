...
public class Postdoc extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static Postdoc currentInstance = null;
	private static final Log log = LogFactory.getLog(Postdoc.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			PostdocIterator thePostdocIterator = (PostdocIterator) findAncestorWithClass(this, PostdocIterator.class);

			if (thePostdocIterator != null) {
				subjectURI = thePostdocIterator.getSubjectURI();
				label = thePostdocIterator.getLabel();
			}

			if (thePostdocIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in Postdoc doStartTag", e);
			throw new JspTagException("Exception raised in Postdoc doStartTag");
		} finally {
			freeConnection();
		}
