package edu.uiowa.slis.VIVOISF.WorkingPaper;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class WorkingPaperCitesType extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static WorkingPaperCitesType currentInstance = null;
	private static final Log log = LogFactory.getLog(WorkingPaperCitesType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			WorkingPaperCitesIterator theWorkingPaperCitesIterator = (WorkingPaperCitesIterator)findAncestorWithClass(this, WorkingPaperCitesIterator.class);
			pageContext.getOut().print(theWorkingPaperCitesIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing WorkingPaper for cites tag ", e);
			throw new JspTagException("Error: Can't find enclosing WorkingPaper for cites tag ");
		}
		return SKIP_BODY;
	}
}

