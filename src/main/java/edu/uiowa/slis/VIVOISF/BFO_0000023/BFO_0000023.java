...
public class BFO_0000023 extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static BFO_0000023 currentInstance = null;
	private static final Log log = LogFactory.getLog(BFO_0000023.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			BFO_0000023Iterator theBFO_0000023Iterator = (BFO_0000023Iterator) findAncestorWithClass(this, BFO_0000023Iterator.class);

			if (theBFO_0000023Iterator != null) {
				subjectURI = theBFO_0000023Iterator.getSubjectURI();
				label = theBFO_0000023Iterator.getLabel();
			}
			...
			if (theBFO_0000023Iterator == null && subjectURI == null) {
				throw new JspException("subject URI generation currently not supported");
			} else {
				ResultSet rs = getResultSet(prefix
				+ " SELECT ?labelUS ?labelENG ?label ?labelANY ?foafName ?schemaName ?rdfValue  ?hideFromDisplay ?placeOfPublication ?abbreviation ?RO_0000052 where {"
			...
			log.error("Exception raised in BFO_0000023 doStartTag", e);
			throw new JspTagException("Exception raised in BFO_0000023 doStartTag");
		} finally {
			freeConnection();
		}
