package edu.uiowa.slis.VIVOISF.Note;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class NoteEditorType extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static NoteEditorType currentInstance = null;
	private static final Log log = LogFactory.getLog(NoteEditorType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			NoteEditorIterator theNoteEditorIterator = (NoteEditorIterator)findAncestorWithClass(this, NoteEditorIterator.class);
			pageContext.getOut().print(theNoteEditorIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Note for editor tag ", e);
			throw new JspTagException("Error: Can't find enclosing Note for editor tag ");
		}
		return SKIP_BODY;
	}
}

