...
public class StatuteIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static StatuteIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(StatuteIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://purl.org/ontology/bibo/Statute> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
