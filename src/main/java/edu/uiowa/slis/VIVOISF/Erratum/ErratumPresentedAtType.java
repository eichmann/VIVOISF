package edu.uiowa.slis.VIVOISF.Erratum;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class ErratumPresentedAtType extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static ErratumPresentedAtType currentInstance = null;
	private static final Log log = LogFactory.getLog(ErratumPresentedAtType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			ErratumPresentedAtIterator theErratumPresentedAtIterator = (ErratumPresentedAtIterator)findAncestorWithClass(this, ErratumPresentedAtIterator.class);
			pageContext.getOut().print(theErratumPresentedAtIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Erratum for presentedAt tag ", e);
			throw new JspTagException("Error: Can't find enclosing Erratum for presentedAt tag ");
		}
		return SKIP_BODY;
	}
}

