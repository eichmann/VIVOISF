...
public class Explanatory extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static Explanatory currentInstance = null;
	private static final Log log = LogFactory.getLog(Explanatory.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			ExplanatoryIterator theExplanatoryIterator = (ExplanatoryIterator) findAncestorWithClass(this, ExplanatoryIterator.class);

			if (theExplanatoryIterator != null) {
				subjectURI = theExplanatoryIterator.getSubjectURI();
				label = theExplanatoryIterator.getLabel();
			}
			...
			if (theExplanatoryIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in Explanatory doStartTag", e);
			throw new JspTagException("Exception raised in Explanatory doStartTag");
		} finally {
			freeConnection();
		}
