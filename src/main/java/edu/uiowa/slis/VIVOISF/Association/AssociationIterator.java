...
public class AssociationIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static AssociationIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(AssociationIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://vivoweb.org/ontology/core#Association> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
