package edu.uiowa.slis.VIVOISF.AcademicArticle;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class AcademicArticleDistributor extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static AcademicArticleDistributor currentInstance = null;
	private static final Log log = LogFactory.getLog(AcademicArticleDistributor.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			AcademicArticleDistributorIterator theAcademicArticleDistributorIterator = (AcademicArticleDistributorIterator)findAncestorWithClass(this, AcademicArticleDistributorIterator.class);
			pageContext.getOut().print(theAcademicArticleDistributorIterator.getDistributor());
		} catch (Exception e) {
			log.error("Can't find enclosing AcademicArticle for distributor tag ", e);
			throw new JspTagException("Error: Can't find enclosing AcademicArticle for distributor tag ");
		}
		return SKIP_BODY;
	}
}

