package edu.uiowa.slis.VIVOISF.Relationship;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class RelationshipPublisherType extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static RelationshipPublisherType currentInstance = null;
	private static final Log log = LogFactory.getLog(RelationshipPublisherType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			RelationshipPublisherIterator theRelationshipPublisherIterator = (RelationshipPublisherIterator)findAncestorWithClass(this, RelationshipPublisherIterator.class);
			pageContext.getOut().print(theRelationshipPublisherIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Relationship for publisher tag ", e);
			throw new JspTagException("Error: Can't find enclosing Relationship for publisher tag ");
		}
		return SKIP_BODY;
	}
}

