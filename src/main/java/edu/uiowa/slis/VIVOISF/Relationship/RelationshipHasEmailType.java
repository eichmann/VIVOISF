package edu.uiowa.slis.VIVOISF.Relationship;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class RelationshipHasEmailType extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static RelationshipHasEmailType currentInstance = null;
	private static final Log log = LogFactory.getLog(RelationshipHasEmailType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			RelationshipHasEmailIterator theRelationshipHasEmailIterator = (RelationshipHasEmailIterator)findAncestorWithClass(this, RelationshipHasEmailIterator.class);
			pageContext.getOut().print(theRelationshipHasEmailIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Relationship for hasEmail tag ", e);
			throw new JspTagException("Error: Can't find enclosing Relationship for hasEmail tag ");
		}
		return SKIP_BODY;
	}
}

