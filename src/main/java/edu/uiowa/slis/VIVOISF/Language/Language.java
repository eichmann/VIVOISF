...
public class Language extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theLanguageIterator != null) {
				subjectURI = theLanguageIterator.getSubjectURI();
				label = theLanguageIterator.getLabel();
			}

			if (theLanguageIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in Language doStartTag", e);
			throw new JspTagException("Exception raised in Language doStartTag");
		} finally {
			freeConnection();
		}
