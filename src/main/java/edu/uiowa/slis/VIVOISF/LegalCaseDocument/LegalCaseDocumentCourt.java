package edu.uiowa.slis.VIVOISF.LegalCaseDocument;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class LegalCaseDocumentCourt extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static LegalCaseDocumentCourt currentInstance = null;
	private static final Log log = LogFactory.getLog(LegalCaseDocumentCourt.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			LegalCaseDocumentCourtIterator theLegalCaseDocumentCourtIterator = (LegalCaseDocumentCourtIterator)findAncestorWithClass(this, LegalCaseDocumentCourtIterator.class);
			pageContext.getOut().print(theLegalCaseDocumentCourtIterator.getCourt());
		} catch (Exception e) {
			log.error("Can't find enclosing LegalCaseDocument for court tag ", e);
			throw new JspTagException("Error: Can't find enclosing LegalCaseDocument for court tag ");
		}
		return SKIP_BODY;
	}
}

