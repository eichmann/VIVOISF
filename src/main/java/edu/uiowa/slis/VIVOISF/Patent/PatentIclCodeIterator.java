...
public class PatentIclCodeIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static PatentIclCodeIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PatentIclCodeIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(Prefix_1_4+"SELECT ?s where { <" + subjectURI + "> <http://vivoweb.org/ontology/core#iclCode> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				iclCode = sol.get("?s").toString();
