...
public class BillIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static BillIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(BillIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://purl.org/ontology/bibo/Bill> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
