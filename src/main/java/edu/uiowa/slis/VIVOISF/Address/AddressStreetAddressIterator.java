...
public class AddressStreetAddressIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static AddressStreetAddressIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(AddressStreetAddressIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(Prefix_1_4+"SELECT ?s where { <" + subjectURI + "> <http://www.w3.org/2006/vcard/ns#streetAddress> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				streetAddress = sol.get("?s").toString();
