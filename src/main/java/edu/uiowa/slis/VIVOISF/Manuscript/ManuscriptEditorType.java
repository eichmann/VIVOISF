package edu.uiowa.slis.VIVOISF.Manuscript;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class ManuscriptEditorType extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static ManuscriptEditorType currentInstance = null;
	private static final Log log = LogFactory.getLog(ManuscriptEditorType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			ManuscriptEditorIterator theManuscriptEditorIterator = (ManuscriptEditorIterator)findAncestorWithClass(this, ManuscriptEditorIterator.class);
			pageContext.getOut().print(theManuscriptEditorIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Manuscript for editor tag ", e);
			throw new JspTagException("Error: Can't find enclosing Manuscript for editor tag ");
		}
		return SKIP_BODY;
	}
}

