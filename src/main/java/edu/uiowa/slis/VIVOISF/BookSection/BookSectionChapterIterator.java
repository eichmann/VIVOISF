...
public class BookSectionChapterIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static BookSectionChapterIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(BookSectionChapterIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(Prefix_1_4+"SELECT ?s where { <" + subjectURI + "> <http://purl.org/ontology/bibo/chapter> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				chapter = sol.get("?s").toString();
