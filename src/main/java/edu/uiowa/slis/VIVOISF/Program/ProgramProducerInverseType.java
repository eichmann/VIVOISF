package edu.uiowa.slis.VIVOISF.Program;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class ProgramProducerInverseType extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static ProgramProducerInverseType currentInstance = null;
	private static final Log log = LogFactory.getLog(ProgramProducerInverseType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			ProgramProducerInverseIterator theProgramProducerInverseIterator = (ProgramProducerInverseIterator)findAncestorWithClass(this, ProgramProducerInverseIterator.class);
			pageContext.getOut().print(theProgramProducerInverseIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Program for producer tag ", e);
			throw new JspTagException("Error: Can't find enclosing Program for producer tag ");
		}
		return SKIP_BODY;
	}
}

