...
public class Category extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theCategoryIterator != null) {
				subjectURI = theCategoryIterator.getSubjectURI();
				label = theCategoryIterator.getLabel();
			}

			if (theCategoryIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in Category doStartTag", e);
			throw new JspTagException("Exception raised in Category doStartTag");
		} finally {
			freeConnection();
		}
