package edu.uiowa.slis.VIVOISF.Report;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class ReportHasAddress extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static ReportHasAddress currentInstance = null;
	private static final Log log = LogFactory.getLog(ReportHasAddress.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			ReportHasAddressIterator theReportHasAddressIterator = (ReportHasAddressIterator)findAncestorWithClass(this, ReportHasAddressIterator.class);
			pageContext.getOut().print(theReportHasAddressIterator.getHasAddress());
		} catch (Exception e) {
			log.error("Can't find enclosing Report for hasAddress tag ", e);
			throw new JspTagException("Error: Can't find enclosing Report for hasAddress tag ");
		}
		return SKIP_BODY;
	}
}

