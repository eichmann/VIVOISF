...
public class IndividualAnniversaryIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static IndividualAnniversaryIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(IndividualAnniversaryIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(Prefix_1_4+"SELECT ?s where { <" + subjectURI + "> <http://www.w3.org/2006/vcard/ns#anniversary> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				anniversary = sol.get("?s").toString();
