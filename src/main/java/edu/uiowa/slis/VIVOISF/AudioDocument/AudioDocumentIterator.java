...
public class AudioDocumentIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static AudioDocumentIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(AudioDocumentIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://purl.org/ontology/bibo/AudioDocument> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
