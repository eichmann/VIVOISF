...
public class ReferenceSourceIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static ReferenceSourceIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(ReferenceSourceIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://purl.org/ontology/bibo/ReferenceSource> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
