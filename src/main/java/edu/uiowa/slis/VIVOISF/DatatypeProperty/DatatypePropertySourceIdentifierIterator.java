...
public class DatatypePropertySourceIdentifierIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static DatatypePropertySourceIdentifierIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(DatatypePropertySourceIdentifierIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://aims.fao.org/aos/geopolitical.owl#sourceIdentifier> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				sourceIdentifier = sol.get("?s").isLiteral() ? sol.get("?s").asLiteral().getString() : sol.get("?s").toString();
				firstInstance = true;
	...
	public void setSourceIdentifier(String theSourceIdentifier) {
		sourceIdentifier = theSourceIdentifier;
	}
