...
public class DatatypePropertySourceModifiedIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static DatatypePropertySourceModifiedIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(DatatypePropertySourceModifiedIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://aims.fao.org/aos/geopolitical.owl#sourceModified> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				sourceModified = sol.get("?s").isLiteral() ? sol.get("?s").asLiteral().getString() : sol.get("?s").toString();
				firstInstance = true;
	...
	public void setSourceModified(String theSourceModified) {
		sourceModified = theSourceModified;
	}
