...
public class FunctionalPropertyExampleAnnotIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static FunctionalPropertyExampleAnnotIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(FunctionalPropertyExampleAnnotIterator.class);

	static boolean firstInstance = false;
	static boolean lastInstance = false;

	String subjectURI = null;
	String exampleAnnot = null;
	ResultSet rs = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://vitro.mannlib.cornell.edu/ns/vitro/0.7#exampleAnnot> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				exampleAnnot = sol.get("?s").isLiteral() ? sol.get("?s").asLiteral().getString() : sol.get("?s").toString();
				firstInstance = true;
	...
	public void setExampleAnnot(String theExampleAnnot) {
		exampleAnnot = theExampleAnnot;
	}
