...
public class ObjectPropertyEditorialNoteIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theObjectProperty != null) {
				subjectURI = theObjectProperty.getSubjectURI();
			}

			if (theObjectProperty == null && subjectURI == null) {
				throw new JspException("subject URI generation currently not supported");
			}

			rs = getResultSet(prefix+"SELECT ?s ?t where {"
					+" <" + subjectURI + "> <http://www.w3.org/2008/05/skos#editorialNote> ?s . "
			...
			while(rs.hasNext()) {
				...
				editorialNote = sol.get("?s").isLiteral() ? sol.get("?s").asLiteral().getString() : sol.get("?s").toString();
				type = sol.get("?t") == null ? null : getLocalName(sol.get("?t").toString());
//				if (type == null)
//					continue;
				if (classFilter == null || (classFilter != null && type != null && classFilter.containsKey(type))) {
					log.info("instance: " + editorialNote + "	type: " + type);
					firstInstance = true;
			...
			log.error("Exception raised in ObjectPropertyEditorialNoteIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in ObjectPropertyEditorialNoteIterator doStartTag");
		}
...
//					continue;
			...
			log.error("Exception raised in ObjectPropertyEditorialNoteIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in ObjectPropertyEditorialNoteIterator doAfterBody");
		}
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			log.error("Exception raised in ObjectPropertyEditorialNote doEndTag", e);
			throw new JspTagException("Exception raised in ObjectPropertyEditorialNote doEndTag");
		} finally {
			clearServiceState();
			freeConnection();
		}
	...
	public void setClassFilter(String filterString) {
		...
		for (String filterClass : classFilterArray) {
			log.info("adding filterClass " + filterClass + " to ObjectPropertyEditorialNoteIterator");
