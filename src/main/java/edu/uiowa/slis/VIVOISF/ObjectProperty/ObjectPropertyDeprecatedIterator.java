...
public class ObjectPropertyDeprecatedIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static ObjectPropertyDeprecatedIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(ObjectPropertyDeprecatedIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.w3.org/2002/07/owl#deprecated> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				deprecated = sol.get("?s").isLiteral() ? sol.get("?s").asLiteral().getString() : sol.get("?s").toString();
				firstInstance = true;
	...
	public void setDeprecated(String theDeprecated) {
		deprecated = theDeprecated;
	}
