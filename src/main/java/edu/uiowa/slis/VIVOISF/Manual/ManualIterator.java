...
public class ManualIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static ManualIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(ManualIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://purl.org/ontology/bibo/Manual> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
