...
public class UndergraduateStudentIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static UndergraduateStudentIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(UndergraduateStudentIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://vivoweb.org/ontology/core#UndergraduateStudent> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
