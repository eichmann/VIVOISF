...
public class EditedBookIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static EditedBookIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(EditedBookIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://purl.org/ontology/bibo/EditedBook> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
