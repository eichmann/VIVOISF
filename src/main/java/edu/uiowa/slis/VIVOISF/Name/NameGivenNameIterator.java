...
public class NameGivenNameIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static NameGivenNameIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(NameGivenNameIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.w3.org/2006/vcard/ns#givenName> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				givenName = sol.get("?s").isLiteral() ? sol.get("?s").asLiteral().getString() : sol.get("?s").toString();
				firstInstance = true;
