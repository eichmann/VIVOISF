...
public class geographical_regionIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static geographical_regionIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(geographical_regionIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://aims.fao.org/aos/geopolitical.owl#geographical_region> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
