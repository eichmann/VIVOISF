package edu.uiowa.slis.VIVOISF.Foundation;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class FoundationInterviewer extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static FoundationInterviewer currentInstance = null;
	private static final Log log = LogFactory.getLog(FoundationInterviewer.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			FoundationInterviewerIterator theFoundationInterviewerIterator = (FoundationInterviewerIterator)findAncestorWithClass(this, FoundationInterviewerIterator.class);
			pageContext.getOut().print(theFoundationInterviewerIterator.getInterviewer());
		} catch (Exception e) {
			log.error("Can't find enclosing Foundation for interviewer tag ", e);
			throw new JspTagException("Error: Can't find enclosing Foundation for interviewer tag ");
		}
		return SKIP_BODY;
	}
}

