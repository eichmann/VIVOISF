package edu.uiowa.slis.VIVOISF.Journal;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class JournalHasURL extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static JournalHasURL currentInstance = null;
	private static final Log log = LogFactory.getLog(JournalHasURL.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			JournalHasURLIterator theJournalHasURLIterator = (JournalHasURLIterator)findAncestorWithClass(this, JournalHasURLIterator.class);
			pageContext.getOut().print(theJournalHasURLIterator.getHasURL());
		} catch (Exception e) {
			log.error("Can't find enclosing Journal for hasURL tag ", e);
			throw new JspTagException("Error: Can't find enclosing Journal for hasURL tag ");
		}
		return SKIP_BODY;
	}
}

