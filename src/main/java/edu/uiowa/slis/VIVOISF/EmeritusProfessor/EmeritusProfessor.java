...
public class EmeritusProfessor extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static EmeritusProfessor currentInstance = null;
	private static final Log log = LogFactory.getLog(EmeritusProfessor.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			EmeritusProfessorIterator theEmeritusProfessorIterator = (EmeritusProfessorIterator) findAncestorWithClass(this, EmeritusProfessorIterator.class);

			if (theEmeritusProfessorIterator != null) {
				subjectURI = theEmeritusProfessorIterator.getSubjectURI();
				label = theEmeritusProfessorIterator.getLabel();
			}

			if (theEmeritusProfessorIterator == null && subjectURI == null) {
				throw new JspException("subject URI generation currently not supported");
			} else {
				ResultSet rs = getResultSet(prefix
				+ " SELECT ?label ?foafName ?schemaName ?rdfValue  ?teachingOverview ?outreachOverview ?researchOverview ?overview where {"
			...
			log.error("Exception raised in EmeritusProfessor doStartTag", e);
			throw new JspTagException("Exception raised in EmeritusProfessor doStartTag");
		} finally {
			freeConnection();
		}
