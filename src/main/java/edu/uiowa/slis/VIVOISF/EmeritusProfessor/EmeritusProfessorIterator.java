...
public class EmeritusProfessorIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static EmeritusProfessorIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(EmeritusProfessorIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://vivoweb.org/ontology/core#EmeritusProfessor> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
