...
public class CommentIterator extends edu.uiowa.slis.VIVOISF.TagLibSupport {
	static CommentIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(CommentIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://purl.org/spar/fabio/Comment> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
